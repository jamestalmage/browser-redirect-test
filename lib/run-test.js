      ...
      common.REDIRECT_CODES.forEach(function(code) {
        ...
        function makeFirstRequest() {
          return new Promise(function(resolve, reject) {
            var opts = makeOpts(method, '/' + id + '/first/' + code);
            http.request(opts, concatJson(resolve, reject)).on('error', reject).end();
          });
        }
...
function concatJson(_resolve, reject) {
  return function _concatJson(res) {
