...
function setup(port)  {
  ...
  app.get('/iconUrl/:agent', function(req, res) {
    var fileInfo = browserIcon(req.params.agent);
    if (fileInfo) {
      res.json(path.relative(baseDirectory, fileInfo.path));
    } else {
      res.sendStatus(404);
    }
  });
  ...
  function sendIcon(agent, res) {
    ...
    if (fileinfo) {
      ...
      fs.createReadStream(browserIcon.DEFAULT).pipe(res.status(404));
    }
  }
