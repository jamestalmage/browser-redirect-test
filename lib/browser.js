...
var filterDataset = require('./filter-dataset');
...
function RunTestController ($location, $http) {
  ...
  this.view = function() {
    $http.get('/testData').then(function(result) {
      ...
      self.unfilteredFlatData = flatten.flattenAggregateData(self.viewData);
  ...
  this.updateFilter = function() {
    var self = this;
    self.flatData = filterDataset(
      self.unfilteredFlatData,
      function(agentNick){
        return self.filter[agentNick];
      }
    );
    console.log('flatData', self.flatData);
  };

  this.universal = function(method, code) {
    return _universal(method, code, 'firstRun') && _universal(method, code, 'secondRun');
  };

  function _universal(method, code, run) {
    var fd = self.flatData && self.flatData[run];
    return fd && fd[method] && fd[method][code] && fd[method][code].$allTheSame;
  }
